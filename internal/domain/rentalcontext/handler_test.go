...
func TestApplyRentalContext(t *testing.T) {
	...
	modified := apply(s, &ModifyRentalContextCommand{PickupTime: &newPickup}, nil, oldPickup)
	...
	modified = apply(s, &ModifyRentalContextCommand{}, locationValue, oldPickup)
...
func TestBuildExtractionInputUsesOnlyRecentRentalHistory(t *testing.T) {
	...
	input := handler.buildExtractionInput(s, "提前一天取车")
